...
public class CreateMeetingIntentHandler extends BaseHandler {
	
    @Override
    public boolean canHandle(HandlerInput input) {
        return input.matches(intentName(IntentNameConstants.CUSTOM_CREATE_MEETINGS));
    }
	...
	public Optional<Response> handle(HandlerInput input) {    	
		...
		Slot receipients = slotMap.get(SlotNameConstants.RECEIPIENTS);		
		String speechText = SpeechTextsConstants.MEETING_SPEECH_TEXT + subject.getValue().toString()
				+ SpeechTextsConstants.START_TIME + startTime.getValue().toString() + SpeechTextsConstants.END_TIME
				+ endTime.getValue().toString() + SpeechTextsConstants.RECEIPIENTS
				+ receipients.getResolutions().getResolutionsPerAuthority().get(0).getValues().get(0).getValue().getName().toString()
				+ SpeechTextsConstants.LOCATION + location.getValue().toString()
				+ SpeechTextsConstants.PAUSE +SpeechTextsConstants.STOP_SPEECH_TEXT;
		
		return buildResponse(input, speechText, GeneralConstants.MEETINGS_CARD_TITLE, true);
	}
